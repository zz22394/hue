...
describe('elasticsearchAutocompleteParser.js', () => {
  ...
  describe('partial removal', () => {
    it('should identify part lengths', () => {
      ...
      expect(elasticsearchAutocompleteParser.identifyPartials('', '')).toEqual({ left: 0, right: 0 });
      expect(elasticsearchAutocompleteParser.identifyPartials('foo', '')).toEqual({ left: 3, right: 0 });
      expect(elasticsearchAutocompleteParser.identifyPartials(' foo', '')).toEqual({ left: 3, right: 0 });
      ...
      expect(elasticsearchAutocompleteParser.identifyPartials('fo', 'o(')).toEqual({ left: 2, right: 2 });
      expect(elasticsearchAutocompleteParser.identifyPartials('fo', 'o(bla bla)')).toEqual({
        left: 2,
        right: 10
      });

      expect(elasticsearchAutocompleteParser.identifyPartials('foo ', '')).toEqual({ left: 0, right: 0 });
      ...
      limitChars.forEach(char => {
        ...
        expect(elasticsearchAutocompleteParser.identifyPartials('bar foo' + char + 'foofoo', '')).toEqual(
          {
            left: 6,
            right: 0
          }
